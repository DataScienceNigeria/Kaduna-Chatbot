...
def get_categories(query: str) -> str:
    ...
    sub_cat = run_query_save_results(
        db, "SELECT DISTINCT health_facility FROM master_microplan"
    )
    cat = run_query_save_results(
        db, "SELECT DISTINCT settlement FROM master_microplan"
    )
    category_str = (
        "List of unique values of the health_facility column : \n"
        + json.dumps(cat, ensure_ascii=False)
    )
    sub_category_str = (
        "\n List of unique values of the settlement column : \n"
